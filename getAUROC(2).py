...
6. Confusion Matrix
'''
parent_dir_5 = "E:\\Courses\\Repo\PORTIA\\Raw_results\\5_mr\\"
parent_dir_40 = "E:\\Courses\\Repo\PORTIA\\Raw_results\\40_mr\\"
parent_dir_100 = "E:\\Courses\\Repo\PORTIA\\Raw_results\\100_mr\\"
if __name__ == '__main__':
    ...
    for f in os.listdir("Raw_results/5_mr"):
    # valid_ranking_df = pd.read_csv("100_mr_sort_result.csv")
    # 读取未排序的result
        predicted_file = os.path.join(parent_dir_5,f)  # 替换为预测结果文件路径
        ranking_df = pd.read_csv(predicted_file, header=None, delimiter = '\t')
        ...
        df_5.to_csv("Results/AUPRC/AUPRC_AUROC_5.csv", index=False)
        
     # Get the predict label
    df_40 = pd.DataFrame(columns = ['File','AUPRC', 'AUROC'])
    for f in os.listdir("Raw_results/40_mr"):
    # valid_ranking_df = pd.read_csv("100_mr_sort_result.csv")
    # 读取未排序的result
        predicted_file = os.path.join(parent_dir_40,f)  # 替换为预测结果文件路径
        ranking_df = pd.read_csv(predicted_file, header=None, delimiter = '\t')
        ranking_df.columns = ['TF', 'Target','Score']
        # print(ranking_df)
        ...
        df_40.to_csv("Results/AUPRC/AUPRC_AUROC_40.csv", index=False)
        
     # Get the predict label
    df_100 = pd.DataFrame(columns = ['File','AUPRC', 'AUROC'])
    for f in os.listdir("Raw_results/100_mr"):
    # valid_ranking_df = pd.read_csv("100_mr_sort_result.csv")
    # 读取未排序的result
        predicted_file = os.path.join(parent_dir_100,f)  # 替换为预测结果文件路径
        ranking_df = pd.read_csv(predicted_file, header=None, delimiter = '\t')
        ranking_df.columns = ['TF', 'Target','Score']
        # print(ranking_df)
        ...
        df_100.to_csv("Results/AUPRC/AUPRC_AUROC_100.csv", index=False)
